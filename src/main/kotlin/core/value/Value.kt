...
data class Value(
    ...
    fun pow(other: Double): Value {
        return Value(
            this.data.pow(other),
            _prev = setOf(this),
            _backward = { this.grad += (other * this.data.pow(other - 1)) * it.grad })
    }

    fun relu(): Value {
        return Value(
            if (data > 0) data else 0.0,
            _prev = setOf(this),
            _backward = { this.grad += it.grad * if (it.data > 0) 1.0 else 0.0 })
    }
